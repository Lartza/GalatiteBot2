...
def get_hit_message(mention: str) -> str:
    hit_rnd = random.randint(0, 24)
    if hit_rnd == 0:
        return (
            f"-PONG! {mention} hit the ball so hard :muscle:, {mention}'s racket broke. "
            f"But {mention} has a spare racket. \nThe game continues. :ping_pong:"
        )
    if hit_rnd == 1:
        return (
            f"-Plash!... {mention} just used a fish :fish: to hit the ball... Guess that counts. \n"
            "The game continues! :ping_pong:"
        )
    if hit_rnd == 2:
        return (
            f"-Stop. **Rigurt the Brash** needs {mention}'s help. He ordered fitting Ping-Pong "
    ...
    if hit_rnd == 3:
        return f"""-pong. {mention} hit the ball! :ping_pong:. \n
    ...
    if hit_rnd == 4:
        return (
            f"-PONG!!!! {mention} gets angry and triggers his Super Saiyajin Mode "
